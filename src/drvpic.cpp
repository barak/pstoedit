 ...
 */
#if 0
drvPIC::drvPIC(const char * driveroptions_p,ostream & theoutStream,ostream & theerrStream): // Constructor
	drvbase(driveroptions_p,theoutStream,theerrStream,
		...
		USE_SPLINE, // do we support B-slines
		1  // BUG: if backend supports elements with fill and edges
	),
#endif
drvPIC::derivedConstructor(drvPIC):
	constructBase,
	troff_mode(0),
	landscape(0),
	keep_font(0),
...
{
	if (landscape) return (y + y_offset) / PIC_SCALE;
	else return (x + x_offset) / PIC_SCALE;
}

float drvPIC::y_coord(float x, float y)
{
	if (landscape) return pageheight - (x + x_offset) / PIC_SCALE;
	else return (y + y_offset) / PIC_SCALE;
}
	...
	for (unsigned int n = 0; n < numberOfElementsInPath(); n++) {
		...
		switch (elem.getType()) {
			...
			case lineto: {
				const Point & p = elem.getPoint(0);
				if (!within_line) {
				     errf << "line from no starting point"
					  << endl;
				}
				y = y_coord(p.x_,p.y_);
				outf << " to "
			...
			case closepath:
				// do this "manually"...
				outf << " to "
				     << x_coord(move_x,move_y) << ","
				     << y_coord(move_x,move_y);
				break;
			default:
...
void drvPIC::show_text(const TextInfo & textinfo)
{
	...
	int pointsize = (textinfo.currentFontSize < 2.0) ?
			0 :
			(int)(textinfo.currentFontSize + 1.8);
	...
	static char selected_font[80];	// Could / should these be class members (wogl???)
	...
	if (is_text) {
		...
		if (strncmp(tfont,selected_font,sizeof(selected_font)-1) != 0) {
			outf << ".ft " << tfont << endl;
		...
		if (debug) {
			...
			outf << ".\\\" currentRGB: " << textinfo.currentR << ","
						     << textinfo.currentG << ","
						     << textinfo.currentB << endl;
		}

		outf << '"';
		if (pointsize) outf << "\\s" << pointsize;
		outf << "\\f";
		// groff/troff style font name
		if (strlen(tfont) > 1) outf << '[' << tfont << ']';
		else outf << tfont;
...
static DriverDescriptionT<drvPIC> D_PIC("pic","PIC format for troff et.al.","pic",
		...
		true); // if format supports multiple pages in one file (wogl???)
