...
drvMPOST::derivedConstructor(drvMPOST):
  ...
  prevFontName(""),
  prevR(0.0f), prevG(0.0f),prevB(0.0f),        // Black
  prevFontAngle( FLT_MAX),
  prevFontSize(-1.0),
  prevLineWidth(0.0f),
...
void drvMPOST::show_text(const TextInfo & textinfo)
{
  ...
  // Change colors
  if (textinfo.currentR != prevR ||
      textinfo.currentG != prevG ||
      textinfo.currentB != prevB) {
    outf << "drawoptions (withcolor ("
         << textinfo.currentR << ',' << textinfo.currentG << ',' << textinfo.currentB
         << "));" << endl;
  ...
  if (textinfo.currentFontSize != prevFontSize) {
    if (textinfo.currentFontSize > 0) {
      ...
      if (verbose)
	errf << "warning: font size of " << textinfo.currentFontSize
	     << "pt encountered; ignoring" << endl;
      outf << "%defaultscale := " << textinfo.currentFontSize
           << "/fontsize defaultfont;" << endl;
    }
  }
  ...
  outf << "showtext ((" << textinfo.x << ',' << textinfo.y << "), "
       << prevFontAngle << ", \"";
...
void drvMPOST::show_path()
{
  ...
  if (sscanf (currentDashPattern, "[ ] %f", &dashOffset) == 1)
    ...
    if (sscanf (currentDashPattern, "[%lu] %f", &lengthOn, &dashOffset) == 2) {
      if (dashOffset)
        sprintf (temptext, " dashed evenly scaled %lubp shifted -%fbp",
                 lengthOn, dashOffset);
      else
      ...
      if (sscanf (currentDashPattern, "[%lu %lu] %f", &lengthOn, &lengthOff, &dashOffset) == 3) {
        if (dashOffset)
          sprintf (temptext, " dashed dashpattern(on %lubp off %lubp) shifted (-%f,0)",
                   lengthOn, lengthOff, dashOffset);
        else
          sprintf (temptext, " dashed dashpattern(on %lubp off %lubp)",
                   lengthOn, lengthOff);
        prevDashPattern = temptext;
      }
      else {
        if (verbose)
          cerr << "Dash pattern \"" << currentDashPattern << "\" is too complex;\n"
  ...
  fillmode = (currentShowType()==drvbase::eofill ||
              currentShowType()==drvbase::fill);
  if (!fillmode && currentShowType()!=drvbase::stroke) {
    errf << "\t\tFatal: unexpected show type " << (int) currentShowType() << " in drvmpost" << endl;
    abort();
  }
...
static DriverDescriptionT<drvMPOST> D_mpost(
		"mpost","MetaPost Format","mp",
