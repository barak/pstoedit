...
static inline float RND3(const float f) { return rnd(f,1000.0f);}
...
// I haven't seen a working example using the /BaseEncoding feature
    ...
    outf << "%/OE/Scaron/Ydieresis/Zcaron/dotlessi/lslash/scaron/zcaron/.notdef/.notdef/.notdef" << endl;
    outf << "%]" << endl;
#endif
...
outf << "255 /ydieresis" << endl;
    ...
    outf << " ] " << endl;
    outf << "/MediaBox [0 0 " << (int) (currentDeviceWidth+0.5) << ' ' << (int) (currentDeviceHeight +0.5) << "]" << endl;
#if 0
...
//    buffer.unsetf(ios::showpoint);
    ...
    buffer << RND3(Sx * cosphi)  << " "
	   ...
	   << RND3(-Sy * sinphi) << " "
	   << RND3(Sy * cosphi)  << " "
	   << RND3(textinfo.x + x_offset) << " "
	   << RND3(textinfo.y + y_offset) << " Tm" << endl;
    buffer << RND3(textinfo.currentR) << " " << RND3(textinfo.currentG) << " " << RND3(textinfo.currentB) << " rg" << endl;
#ifdef TFALWAYSONE
    buffer << RND3(textinfo.cx/textinfo.currentFontSize) << ' ' << RND3(textinfo.ax/textinfo.currentFontSize) << ' ';
#else
...
//    buffer.unsetf(ios::showpoint);
    ...
    buffer << RND3(currentR()) << " " << RND3(currentG()) << " " << RND3(currentB()) << " " << setrgbcolor << endl;
