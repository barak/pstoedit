/* 
   ...
   Implementation of KIllustrator output driver.
...
#if 0
drvKIllustrator::drvKIllustrator (const char *driveroptions_p,
				  ostream& theoutStream,
				  ostream& theerrStream): // Constructor
...
#endif
drvKIllustrator::derivedConstructor(drvKIllustrator):
	constructBase
{
// driver specific initializations
// and writing of header to output file
  outf << "<?xml version=\"1.0\"?>\n"
       << "<doc editor=\"KIllustrator\" mime=\"application/x-killustrator\" >\n"
       << "<head>\n"
...
void drvKIllustrator::print_coords () {
  for (unsigned int n = 0; n < numberOfElementsInPath (); n++) {
    ...
    break;
    default:
      errf << "\t\tFatal: unexpected case in drvKIllustrator " << endl;
      abort();
      break;
    }
  }
...
void drvKIllustrator::open_page () {
}

void drvKIllustrator::close_page () {
}

void drvKIllustrator::show_text(const TextInfo& textinfo) {
  ...
  outf << "\" point-size=\"" << textinfo.currentFontSize << "\""
       << " weight=\"";
...
void drvKIllustrator::show_path () {
  if (isPolygon ())
    outf << "<polygon ";
  else
    outf << "<polyline ";
...
void drvKIllustrator::show_rectangle (const float llx, const float lly, 
...
int drvKIllustrator::cvtColor (float c) {
  return int ((c * 255.0) + 0.5);
}

static DriverDescriptionT<drvKIllustrator> D_KIllustrator("kil",".kil format for KIllustrator","kil",
