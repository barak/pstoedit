/* 
   ...
   will produce Fig output.  The supported output types are:
      ...
      X, pnm, gif, cgm, ai, ps, fig, pcl, hpgl, tek, meta.
...
enum { PAGE_ANSI_A, PAGE_ANSI_B, PAGE_ANSI_C, PAGE_ANSI_D, PAGE_ANSI_E, PAGE_ISO_A4, PAGE_ISO_A3, PAGE_ISO_A2, PAGE_ISO_A1, PAGE_ISO_A0, PAGE_JIS_B5, PAGE_LEGAL, PAGE_LEDGER };
...
struct page_size known_page_sizes[NUM_LIBPLOT_PAGE_SIZES] = 
{
...
inline void
Plotter::emit_byte (int c)
{
  outstream->put ((unsigned char)c);
}

inline void
Plotter::emit_string (const char *s)
{
  bool has_newline;
...
int
Plotter::fbezier3 (double x0, double y0, double x1, double y1, double x2, double y2, double x3, double y3)
{
...
int
Plotter::fbox (double x0, double y0, double x1, double y1)
{
...
int
Plotter::fconcat (double m0, double m1, double m2, double m3, double m4, double m5)
{
...
int
Plotter::fcont (double x, double y)
{
...
int
Plotter::fillcolor (int red, int green, int blue)
{
...
int
Plotter::fline (double x0, double y0, double x1, double y1)
{
...
int
Plotter::flinedash (int n, const double *dashes, double offset)
{
  int i;
...
int
Plotter::flinewidth (double new_line_width)
{
  emit_byte (meta_portable_output ? (int)O_LINEWIDTH : (int)O_FLINEWIDTH);
...
int
Plotter::fmove (double x, double y)
{
...
int
Plotter::fspace (double x0, double y0, double x1, double y1)
{
...
Plotter::openpl ()
{
  bool not_previously_opened = (opened == false ? true : false);
...
int
Plotter::pencolor (int red, int green, int blue)
{
...
int
dummy_warning_handler (const char *)
{
  return 0;
}
#endif
...
drvplot::derivedConstructor(drvplot): constructBase
{
	  ...
	  if ((i+1) < d_argc)
	    {
	      ...
	      for (j = 0; j < NUM_LIBPLOT_PAGE_SIZES; j++)
		if (strcmp (d_argv[i+1], known_page_sizes[j].name) == 0
		    || 
		    (known_page_sizes[j].alias
		     && 
...
#else
	  errf << "Please use global -pagesize option instead driver specific -page-size option" << endl;
#endif
	}
  ...
  if (strcmp (Pdriverdesc->symbolicname, "plot") == 0)
    {
      if (strcmp (type, "pnm") == 0)
	plotter = new PNMPlotter(cin, outf, errf);
      else if (strcmp (type, "gif") == 0)
	{
	  close_output_file_and_reopen_in_binary_mode ();
...
drvplot::close_output_file_and_reopen_in_binary_mode ()
{
  if (outFileName)
    // output is to a file, and outf is not cout
    {
	  ...
	  outputFilePtr->open (outFileName, ios::out);
#else
	  // use redundant ios::out because of bug in djgpp
	  outputFilePtr->open (outFileName, ios::out | ios::binary);
#endif
    }
}
	...
	case lineto: 
	  {
	    ...
	    if (currently_at_lastpoint)
	      ...
	      plotter->fline (lastpoint.x_ + x_offset, lastpoint.y_ + y_offset,
			      p.x_ + x_offset,  p.y_ + y_offset);
	    lastpoint = p;
...
#define POINTS_PER_INCH 72.0
  ...
  double height 
    = POINTS_PER_INCH * (double)known_page_sizes[page_type].height;
  double width 
    = POINTS_PER_INCH * (double)known_page_sizes[page_type].width;
  ...
  if (physical_page)
	...
	= POINTS_PER_INCH * (double)known_page_sizes[page_type].viewport_size;
      plotter->fspace (0.5 * (width - viewport_size), 
		       0.5 * (height - viewport_size),
		       0.5 * (width + viewport_size), 
		       0.5 * (height + viewport_size));
    }
  else
    ...
    {
      if (height > width)
	plotter->fspace (-0.5 * (height - width), 0.0, 
			 0.5 * (height + width), height);
      else
	plotter->fspace (0.0, -0.5 * (width - height),
			 width, 0.5 * (width + height));
    }
...
static inline int 
plotcolor (const float f) 
{ 
  return (int)(65535 * f);
}

void
drvplot::show_text (const TextInfo& textinfo)
{
  if (strlen (textinfo.thetext.value()) > 0)
    {
      ...
      plotter->pencolor (plotcolor(textinfo.currentR),
			 plotcolor(textinfo.currentG) ,
			 plotcolor(textinfo.currentB));
      float *matrix = getCurrentFontMatrix();
      double sinv;
      if (textinfo.currentFontSize != 0.0)
	sinv = 1.0 / textinfo.currentFontSize;
      else sinv = 0.0;
      plotter->savestate ();
      plotter->fconcat (sinv * (double)matrix[0], sinv * (double)matrix[1],
			sinv * (double)matrix[2], sinv * (double)matrix[3],
			textinfo.x + x_offset, textinfo.y + y_offset);
...
drvplot::set_line_style ()
{
  ...
  plotter->joinmod (currentLineJoin() == 0 ? "miter" :
		    currentLineJoin() == 1 ? "round" :
		    currentLineJoin() == 2 ? "bevel" : "miter" );

  // set old-fashioned line style
  const char *linestyle;
  switch (currentLineType ())
    {
    case solid: default : linestyle = "solid"; break;
    case dashed		: linestyle = "longdashed"; break;
    case dashdot	: linestyle = "dotdashed"; break;
    case dotted		: linestyle = "dotted"; break;
    case dashdotdot     : linestyle = "dotdotdashed"; break;
    }
  ...
  plotter->flinedash (dash_pattern.nrOfEntries, numbers,
		      (double)dash_pattern.offset);
  delete [] numbers;
}
...
// can be drawn.
void
drvplot::set_filling_and_edging_style ()
{
  switch (currentShowType()) 
    {
    case drvbase::stroke : 
      plotter->flinewidth (currentLineWidth());
      plotter->pencolor (plotcolor (currentR ()) ,  
			 plotcolor (currentG ()) ,  
			 plotcolor (currentB ()));
	...
	{
	  plotter->flinewidth (currentLineWidth());
	  plotter->pencolor (plotcolor (edgeR ()) ,
			     plotcolor (edgeG ()) ,  
			     plotcolor (edgeB ()));
	  plotter->fillcolor (plotcolor (fillR ()) ,
			      plotcolor (fillG ()) ,  
			      plotcolor (fillB ()));
	}
      else
	{
	  ...
	  plotter->pencolor (plotcolor (currentR()) ,  
			     plotcolor (currentG()) ,  
			     plotcolor (currentB()));
	  plotter->fillcolor (plotcolor (currentR()) ,  
			      plotcolor (currentG()) ,  
			      plotcolor (currentB()));
	}
      plotter->filltype (1);
...
void
drvplot::show_rectangle (const float llx, const float lly, const float urx, const float ury)
{
  set_line_style ();
  set_filling_and_edging_style ();
...
void
drvplot::show_image(const Image &)
{
  // not implemented
}

static DriverDescriptionT<drvplot> D_plot_meta_a ("gmfa", "ASCII GNU metafile ", "meta",
...
static DriverDescriptionT<drvplot> D_plot_meta_b ("gmfb", "binary GNU metafile ", "meta",
...
#ifdef HAVE_LIBPLOTTER
static DriverDescriptionT<drvplot> D_plot ("plot", "GNU libplot output types, e.g. plot:type X", "plot",
		false,		// backend does not support subpaths
		true,		// backend supports curves
		true,		// backend supports filled elements with edges 
		true,		// backend supports text
